...
//Accessibility
static Eina_Bool __access_apps_view_icon_clicked_cb (void *data, Evas_Object *obj, Elm_Access_Action_Info *event_info)
{
...
void apps_view_show(void)
{
	...
	EINA_LIST_FOREACH(data_list, find_list, item) {
		if (item->app_layout) {
			...
			elm_access_action_cb_set(item->access_obj, ELM_ACCESS_ACTION_ACTIVATE, __access_apps_view_icon_clicked_cb, item);
		}
	}
}
...
static void __apps_view_badge_update_cb(unsigned int action, const char *app_id, unsigned int count, void *user_data)
{
	...
	EINA_LIST_FOREACH(data_list, find_list, item) {
		if (item->app_id && strcmp(app_id, item->app_id) == 0) {
			__apps_view_badge_update_count(item);
		}
	}
}
...
static void __apps_view_badge_update_folder(int parent_db_id)
{
	...
	app_data_t *folder_item = NULL;
	Eina_List *apps_list = apps_data_get_list();
	int badge_count = 0;
	char number_str[STR_MAX];
