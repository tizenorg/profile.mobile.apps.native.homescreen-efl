...
page_indicator_t * page_indictor_create(Evas_Object *scroller)
{
	...
	for (i = 0; i < PAGE_INDICATOR_MAX_PAGE_COUNT; i++) {
		...
		edje_object_signal_callback_add(elm_layout_edje_get(page_indicator->unit[i]), SIGNAL_PAGE_INDICATOR_UNIT_CLICKED, SIGNAL_SOURCE, __page_indicator_unit_clicked_cb, page_indicator);
