...
static int __page_indicator_convert_page_to_indicator(int page_count, int cur_page);
...
static int __page_indicator_convert_page_to_indicator(int page_count, int cur_page)
{
	...
	if (page_count > PAGE_INDICATOR_MAX_PAGE_COUNT) {
		...
		if (cur_page < center_left) {
			indicator = cur_page;
		}
		else if (cur_page >= center_left && cur_page <= center_right) {
			indicator = PAGE_INDICATOR_CENTER_PAGE_INDEX;
		}
		else {
			indicator = cur_page - center_right + PAGE_INDICATOR_CENTER_PAGE_INDEX;
		}
		indicator %= PAGE_INDICATOR_MAX_PAGE_COUNT;
	}
	else {
		indicator = cur_page;
...
static void __page_indicator_scroll_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	cur_page = (x / page_indicator->w) % page_indicator->page_count;
	LOGD("Current x: %d, current Page: %dth", x, cur_page);

	cur_indicator = __page_indicator_convert_page_to_indicator(page_indicator->page_count, cur_page);
	next_indicator = __page_indicator_convert_page_to_indicator(page_indicator->page_count, cur_page + 1);

	from_x = page_indicator->w * cur_page;

	angle = (double)(x - from_x) / page_indicator->w * 90.0;
	LOGD("x: %d, from_x: %d, Angle: %f", x, from_x, angle);

	color = fabs(angle) * 2;
	__page_indicator_unit_rotate(page_indicator->unit[cur_indicator], 90 + angle, 255.0 - color);
	__page_indicator_unit_rotate(page_indicator->unit[next_indicator], 180 + angle, 75.0 + color);
}
