...
static void __cluster_view_add_page(void);
...
Evas_Object *__cluster_view_create_base_gui(Evas_Object *win)
{
    ...
    evas_object_size_hint_align_set(cluster_view_s.box, EVAS_HINT_FILL, EVAS_HINT_FILL);
...
/*
    ...
    evas_object_size_hint_min_set(item->widget_layout, CLUSTER_W, CLUSTER_H);
    ...
    evas_object_resize(item->widget_layout, CLUSTER_W, CLUSTER_H);
...
*/
    __cluster_view_add_page();
    __cluster_view_add_page();
    cluster_view_s.page_count = 2;
...
static void __cluster_view_add_page(void)
{
    ...
    Evas_Object *layout = elm_layout_add(cluster_view_s.box);
    elm_layout_file_set(layout, util_get_res_file_path(EDJE_DIR"/cluster_view.edj"),GROUP_CLUSTER_LY);
    evas_object_size_hint_align_set(layout, EVAS_HINT_FILL, EVAS_HINT_FILL);
    evas_object_size_hint_weight_set(layout, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
    evas_object_resize(layout, CLUSTER_W, CLUSTER_H);
    evas_object_show(layout);

    Evas_Object *page_bg = evas_object_rectangle_add(evas_object_evas_get(layout));
    ...
    evas_object_resize(page_bg, CLUSTER_W, CLUSTER_H);
    evas_object_color_set(page_bg, 0, 0, 0, 10);
    evas_object_show(page_bg);
    elm_object_part_content_set(layout, SIZE_SETTER, page_bg);
    ...
    elm_layout_content_set(layout, CLUSTER_BOX, box);
    ...
    elm_box_pack_end(box, item->widget_layout);
    /*******************************/

    elm_box_pack_end(cluster_view_s.box, layout);
}
